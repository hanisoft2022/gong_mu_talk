...
class CareerDisplayHelper {
  ...
  static String getCompactCareerList(List<String> careerIds, {int maxDisplay = 3}) {
    if (careerIds.isEmpty) return '';

    final displayNames = careerIds.take(maxDisplay).map(getCareerDisplayName).toList();
  ...
  static List<CareerGroup> groupCareers(List<String> careerIds) {
    ...
    // 국가행정직 그룹
    final nationalAdmins = careerIds.where((id) =>
    ...
    // 지방행정직 그룹
    final localAdmins = careerIds.where((id) =>
      id == 'admin_9th_local' ||
      id == 'admin_7th_local' ||
      id == 'admin_5th_local'
    ).toList();
    ...
    // 전문행정직 그룹
    final specializedAdmins = careerIds.where((id) =>
      id == 'tax_officer' ||
      id == 'customs_officer' ||
      id == 'job_counselor' ||
    ...
    // 중등교사 그룹
    final secondaryTeachers = careerIds.where((id) =>
    ...
    // 비교과 교사 그룹
    final nonSubjectTeachers = careerIds.where((id) =>
    ...
    // 보건복지직 그룹
    final healthWelfare = careerIds.where((id) =>
    ...
    // 공안직 그룹
    final publicSecurity = careerIds.where((id) =>
    ...
    // 군인 그룹
    final military = careerIds.where((id) =>
      id == 'army' ||
      id == 'navy' ||
      id == 'air_force' ||
    ...
    // 기술직 - 공업 그룹
    final industrial = careerIds.where((id) =>
    ...
    // 기술직 - 시설환경 그룹
    final facilities = careerIds.where((id) =>
      id == 'civil_engineer' ||
      id == 'architect' ||
    ...
    // 기술직 - 농림수산 그룹
    final agriculture = careerIds.where((id) =>
    ...
    // 기술직 - IT통신 그룹
    final itComm = careerIds.where((id) =>
      id == 'computer_officer' ||
    ...
    // 기술직 - 관리운영 그룹
    final management = careerIds.where((id) =>
    ...
    final ungrouped = careerIds.where((id) => !allGroupedIds.contains(id)).toList();
